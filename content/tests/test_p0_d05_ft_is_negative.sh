#!/bin/bash
# test_p0_d05_ft_is_negative.sh — hash verification
# Usage: bash test_p0_d05_ft_is_negative.sh [source_dir]
set -e

EXERCISE_ID="p0_d05_ft_is_negative"
SRC_DIR="${1:-.}"

echo "=== Testing: ${EXERCISE_ID} ==="

# Check source files exist
if [ ! -f "${SRC_DIR}/ft_is_negative.c" ]; then
    echo "FAILED: File 'ft_is_negative.c' not found"
    exit 1
fi
...
for f in ft_is_negative.c ft_putchar.c; do
...
gcc -Wall -Wextra -Werror -o /tmp/${EXERCISE_ID}_test \
    "${SRC_DIR}/ft_is_negative.c" "${SRC_DIR}/ft_putchar.c" /tmp/${EXERCISE_ID}_main.c
if [ $? -ne 0 ]; then
