#!/bin/bash
# test_p0_d02_ft_putstr.sh — hash verification
# Usage: bash test_p0_d02_ft_putstr.sh [source_dir]
set -e

EXERCISE_ID="p0_d02_ft_putstr"
SRC_DIR="${1:-.}"
...
if grep -qE '\bfor\s*\(' "${SRC_DIR}/ft_putstr.c" 2>/dev/null; then
    echo "FAILED: Forbidden 'for' loop detected in ft_putstr.c (use 'while')"
    exit 1
fi
