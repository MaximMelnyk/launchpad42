#!/bin/bash
# test_c05_ex03_ft_recursive_power.sh — hash verification
# Usage: bash test_c05_ex03_ft_recursive_power.sh [source_dir]
set -e

EXERCISE_ID="c05_ex03_ft_recursive_power"
SRC_DIR="${1:-.}"

echo "=== Testing: ${EXERCISE_ID} ==="
echo "(C05: ft_recursive_power)"
echo ""

# Check source file exists
if [ ! -f "${SRC_DIR}/ft_recursive_power.c" ]; then
    echo "FAILED: File 'ft_recursive_power.c' not found"
    exit 1
fi

# Check for forbidden functions
if grep -qE '\bprintf\b|\bscanf\b|\bputs\s*\(|\bpow\s*\(' "${SRC_DIR}/ft_recursive_power.c" 2>/dev/null; then
    echo "FAILED: Forbidden function in ft_recursive_power.c (printf/scanf/puts/pow)"
    exit 1
fi

# Check for forbidden for loops
if grep -qE '\bfor\s*\(' "${SRC_DIR}/ft_recursive_power.c" 2>/dev/null; then
    echo "FAILED: Forbidden 'for' loop in ft_recursive_power.c (use 'while')"
    exit 1
fi

# Check that it actually uses recursion (function calls itself)
if ! grep -qE 'ft_recursive_power\s*\(' "${SRC_DIR}/ft_recursive_power.c" 2>/dev/null; then
    echo "WARNING: Function may not use recursion (ft_recursive_power not found in body)"
fi

# Norminette check (if available)
if command -v norminette &> /dev/null; then
    echo "Running norminette..."
    norminette "${SRC_DIR}/ft_recursive_power.c" || {
...
int	main(void)
{
	...
	ft_putchar('\n');
	ft_putnbr(ft_recursive_power(100, 0));
	ft_putchar('\n');
...
gcc -Wall -Wextra -Werror -o /tmp/${EXERCISE_ID}_test1 \
    "${SRC_DIR}/ft_recursive_power.c" /tmp/${EXERCISE_ID}_main.c
