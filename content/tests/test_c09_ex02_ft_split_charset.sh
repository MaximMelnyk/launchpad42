#!/bin/bash
# test_c09_ex02_ft_split_charset.sh — hash verification
# Usage: bash test_c09_ex02_ft_split_charset.sh [source_dir]
set -e

EXERCISE_ID="c09_ex02_ft_split_charset"
SRC_DIR="${1:-.}"
...
echo "(C09 ex02: ft_split charset — split by set of delimiters)"
echo ""
...
if grep -qE '\bstrtok\b|\bstrsep\b|\bstrstr\b' "${SRC_DIR}/ft_split.c" 2>/dev/null; then
    echo "FAILED: Forbidden function (strtok/strsep/strstr) in ft_split.c"
    exit 1
fi
...
int	main(void)
{
	...
	/* Single char, no delimiters */
	r = ft_split("x", " ");
	if (r && r[0])
		ft_putstr(r[0]);
	ft_putchar('\n');
	free_split(r);
	/* All chars are delimiters */
	r = ft_split("abcabc", "abc");
	if (r && r[0] == NULL)
		ft_putstr("EMPTY");
	ft_putchar('\n');
	...
	ft_putchar('\n');
	free_split(r);
	/* Multi-char delimiter set */
	r = ft_split("aXbYcZd", "XYZ");
